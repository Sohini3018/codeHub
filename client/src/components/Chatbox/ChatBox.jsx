...
const ChatBox = () => {
  ...
  const handleSend = async () => {
    ...
    //
    socketio?.emit(Actions.CHAT_SEND, {
      content: userInput,
      senderType: "USER",
    });
    ...
    socketio?.emit(Actions.CHAT_SEND, {
      content: responseFromModel[0].generated_text,
      senderType: "BOT",
    });
  ...
  return (
    <div className="flex flex-col h-[90vh]">
      <div className="flex flex-col justify-end p-10 rounded-lg w-full h-full">
        <div
          className="flex flex-col flex-grow overflow-y-auto mb-4 gap-4"
          ref={chatRef}
        >
          {messages &&
            messages.map((message, index) =>
              message.senderType === "USER" ? (
                <MessageTemplate mode="user" text={message.content} />
              ) : (
                <MessageTemplate mode="bot" text={message.content} />
              )
            )}
        </div>
        <div className="flex">
